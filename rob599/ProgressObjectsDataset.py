...
class ProgressObjectsDataset(VisionDataset):
    ...
    url = "https://drive.usercontent.google.com/download?id=1C8_JFsnPVm392C-S1rH0y4HFfNkdMlXi"
    ...
    def download(self) -> None:
        ...
        drive_response = requests.get(self.url)
        drive_soup = BeautifulSoup(drive_response.text, "html.parser")

        form = drive_soup.find('form', attrs={"id": "download-form"})

        download_link = form.attrs['action']
        for node_i, node in enumerate(form.find_all("input", attrs={"type":"hidden"})):
          if node_i==0:
            download_link += "?"
          else: 
            download_link += "&"
          download_link += f"{node.attrs['name']}={node.attrs['value']}"
        
        if self._check_integrity():
            print("Files already downloaded and verified")
            return
        download_and_extract_archive(download_link, self.root, filename=self.filename, md5=self.tgz_md5)
